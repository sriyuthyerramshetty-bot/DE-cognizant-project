        ...
        <>
            ...
            <div className={filteredPlans.length === 0 ? "flex items-center justify-center h-64 px-6" : "grid grid-cols-3 gap-4 px-6"}>
                {filteredPlans.length === 0 ? (
                    <p className="text-center text-2xl text-black-600">{TextAsset.PlanPage.searchError}</p>
                ) : (
                        ...
                        <div key={plan.id} className="relative border p-4 rounded-lg shadow-md flex flex-col h-48 w-full overflow-hidden">
                            ...
                            <div className="flex items-baseline justify-between gap-1 mt-auto">
                                <div className="flex items-baseline gap-1">
                                    <span className="text-4xl text-black-500 font-bold">{plan.price.split('/')[0]}</span>
                                    <span className="text-sm">/{plan.price.split('/')[1]}</span>
                                </div>
                                <button
                                    onClick={() => addToCart(plan)}
                                    disabled={cart.some((p) => p.id === plan.id)}
                                    className={`py-1 px-3 rounded transition-colors flex items-center gap-1 text-sm text-white ${
                                        cart.some((p) => p.id === plan.id)
                                            ? 'bg-gray-400 cursor-not-allowed'
                                            : 'bg-red-500 hover:bg-red-600'
                                    }`}
                                >
                                    {cart.some((p) => p.id === plan.id) ? 'Added' : 'Add to Cart'} <ShoppingCart size={14} />
                                </button>
                            </div>
                        </div>
                    ))
